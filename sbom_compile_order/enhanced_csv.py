...
def _process_one_row(
    ...
    """
    Process one compile-order row into an enhanced row (metadata, POM, etc.).
    Used for parallel execution; ctx holds pom_downloader, locks, paths, and flags.
    """
    if len(row) < 4:
        return (idx, list(row))
    row = list(row)
    log_lock = ctx["log_lock"]
    log_file = ctx["log_file"]
    verbose = ctx["verbose"]

    def log(msg: str) -> None:
        with log_lock:
            _log_to_file(msg, log_file)
            if verbose:
    ...
    existing_enhanced_rows = ctx["existing_enhanced_rows"]
    incremental_update = ctx["incremental_update"]
    ...
    if use_existing_data and existing_row_data and len(existing_row_data) > 13:
        ...
        if verbose and (homepage_url or license_type):
            log(f"Using existing metadata for {group or artifact}:{artifact}:{version} (incremental update mode)")
    elif metadata_client and comp.name and (is_npm or (is_maven and comp.group)):
        try:
            homepage, license = metadata_client.get_package_info(comp)
            if homepage:
                homepage_url = homepage
                log(f"Found homepage for {group or artifact}:{artifact}:{version} {homepage_url}")
            if license:
                license_type = license
                log(f"Found license for {group or artifact}:{artifact}:{version} {license_type}")
            if is_npm:
                try:
                    npm_data = metadata_client.get_comprehensive_npm_data(comp)
                    if npm_data:
                        repo_url_from_npm = _normalize_npm_repo_url(npm_data.get("repository") or "")
                        if repo_url_from_npm and npm_data.get("repository"):
                            log(f"[npm] Normalized repository URL for {artifact}:{version}: {npm_data.get('repository')} -> {repo_url_from_npm}")
                except Exception as npm_data_exc:  # pylint: disable=broad-exception-caught
                    if verbose:
                        log_warn(f"[npm] Note: Could not extract comprehensive data for {artifact}:{version}: {npm_data_exc}")
        except Exception as exc:  # pylint: disable=broad-exception-caught
            log_warn(f"[WARNING] Failed to lookup metadata for {group or artifact}:{artifact}:{version}: {exc}")
    ...
    if pom_downloader and is_maven and comp.group and comp.name and comp.version and not skip_pom_download:
        try:
            ...
            if pom_filename:
                ...
                log(f"Downloaded POM for {group}:{artifact}:{version}: {pom_filename}")
                        ...
                        with open(pom_file_path, "r", encoding="utf-8") as pom_file:
                            ...
                            if scm_url:
                                ...
                                if repo_url_from_pom:
                                    log(f"Extracted repo URL from POM for {group}:{artifact}:{version}: {repo_url_from_pom}")
                    except Exception as pom_read_exc:  # pylint: disable=broad-exception-caught
                        log_warn(f"[WARNING] Failed to read POM file {pom_filename} to extract repo URL: {pom_read_exc}")
            elif auth_req:
                downloaded_status = "Authentication required"
                log(f"Authentication required for POM download: {group}:{artifact}:{version}")
            else:
                downloaded_status = "Failed to download (not found or unavailable)"
                log(f"POM download failed for {group}:{artifact}:{version}: not found or unavailable")
        except Exception as exc:  # pylint: disable=broad-exception-caught
            downloaded_status = f"Error: {str(exc)}"
            log_warn(f"[WARNING] Failed to download POM for {group}:{artifact}:{version}: {exc}")
...
def create_enhanced_csv(
    ...
    Args:
        ...
        max_workers: Number of parallel workers for row processing (1 = sequential)
    """
    if log_file is None:
    ...
    log_msg = f"Enhanced CSV header written, starting incremental processing (max_workers={max_workers})"
    _log_to_file(log_msg, log_file)
    if verbose:
        print(f"[INFO] {log_msg}", file=sys.stderr)
    ...
    if max_workers <= 1:
        ...
        metadata_clients = (
            [PackageMetadataClient(verbose=verbose) for _ in range(max_workers)]
            if metadata_client
        ...
        ctx = {
            ...
            "header": header,
            "existing_enhanced_rows": existing_enhanced_rows,
            "incremental_update": incremental_update,
        }
        tasks = [
            (idx, row, metadata_clients[(idx - 1) % max_workers])
            for idx, row in enumerate(rows, 1)
        ]
        results = {}
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            futures = [ex.submit(_process_one_row, idx, row, mc, ctx) for idx, row, mc in tasks]
        ...
        for i in range(1, len(rows) + 1):
            ...
            os.fsync(enhanced_file.fileno())
            if i % 100 == 0:
                log_msg = f"Processed {i}/{len(rows)} rows"
                _log_to_file(log_msg, log_file)
                if verbose:
                    print(f"[INFO] {log_msg}", file=sys.stderr)
